...
pcall(function()
    ...
    local old = cg:FindFirstChild("UNGHUB")
    if old then old:Destroy() end
end)
-- UNG HUB V1.0
-- Developed for UNG 70% - Modern UI Design
...
-- GUI Setup (UNG HUB V1.0)
local ScreenGui = Instance.new("ScreenGui")
ScreenGui.Name = "UNGHUB"
ScreenGui.ResetOnSpawn = false
ScreenGui.Parent = game:GetService("CoreGui")

local MainFrame = Instance.new("Frame")
MainFrame.Size = UDim2.new(0, 600, 0, 450) -- ขนาดใหญ่ขึ้นสำหรับ UNG
MainFrame.Position = UDim2.new(0.5, -300, 0.5, -225)
MainFrame.BackgroundColor3 = Color3.fromRGB(15, 15, 20) -- สีเข้มสำหรับ UNG
...
border.Color = Color3.fromRGB(70, 130, 180) -- Steel Blue for UNG
border.Thickness = 2
...
Header.TextSize = 24
Header.TextColor3 = Color3.fromRGB(70, 130, 180) -- Steel Blue
...
Subtitle.Text = "Professional Gaming Hub - UNG 70%"
...
ServerTimeLabel.Position = UDim2.new(0.5, 0, 0, 20)
ServerTimeLabel.Size = UDim2.new(0, 250, 0, 25)
ServerTimeLabel.BackgroundTransparency = 1
ServerTimeLabel.Font = Enum.Font.GothamBold
ServerTimeLabel.TextSize = 16
ServerTimeLabel.TextColor3 = Color3.fromRGB(70, 130, 180)
...
sidebarStroke.Color = Color3.fromRGB(70, 130, 180)
...
Content.ScrollBarImageColor3 = Color3.fromRGB(70, 130, 180)
...
MinBtn.MouseButton1Click:Connect(function()
    ...
    -- Smooth animation
    local targetSize = minimized and UDim2.new(0, 500, 0, 60) or UDim2.new(0, 600, 0, 450)
...
local function CreateSidebarButton(text, yPos)
    ...
    Btn.Size = UDim2.new(1, -15, 0, 45) -- ขนาดใหญ่ขึ้นสำหรับ UNG
    Btn.Position = UDim2.new(0, 7, 0, yPos)
    Btn.BackgroundColor3 = Color3.fromRGB(40, 40, 55) -- สีเข้มขึ้น
    Btn.Text = text
    Btn.Font = Enum.Font.GothamBold
    Btn.TextSize = 14 -- ขนาดตัวอักษรใหญ่ขึ้น
    ...
    -- Add hover effects for UNG
    Btn.MouseEnter:Connect(function()
        local tween = TweenService:Create(Btn, TweenInfo.new(0.2, Enum.EasingStyle.Quart), {
...
local MainBtn      = CreateSidebarButton("🏠 Main", 15)
local WeaponBtn    = CreateSidebarButton("⚔️ Weapon", 65)
local PlayerBtn    = CreateSidebarButton("👥 Player", 115)
local DevilFruitBtn= CreateSidebarButton("🍎 DevilFruit", 165)
local MiscBtn      = CreateSidebarButton("⚙️ Misc", 215)
local DrinkBtn     = CreateSidebarButton("🥤 Drink", 265)
local TeleportBtn  = CreateSidebarButton("🚀 Teleport", 315)
local ServerBtn    = CreateSidebarButton("🌐 Server", 365)
...
-- Modern button creation function for UNG
local function CreateModernButton(parent, text, color, size)
    local btn = Instance.new("TextButton", parent)
    btn.Size = size or UDim2.new(1, -20, 0, 45) -- ขนาดใหญ่ขึ้นสำหรับ UNG
    ...
    local corner = Instance.new("UICorner", btn)
    corner.CornerRadius = UDim.new(0, 12) -- มุมโค้งมากขึ้น
    
    -- Add modern hover effects for UNG
    btn.MouseEnter:Connect(function()
        local tween = TweenService:Create(btn, TweenInfo.new(0.2, Enum.EasingStyle.Quart), {
            BackgroundColor3 = Color3.fromRGB(70, 130, 180),
            TextColor3 = Color3.fromRGB(255, 255, 255)
...
-- Enhanced function to change button colors with smooth animation for UNG
local function SetActiveTab(tabName)
    for name, btn in pairs(TabButtons) do
        if name == tabName then
            -- Active tab - Steel Blue color with animation
            local tween = TweenService:Create(btn, TweenInfo.new(0.3, Enum.EasingStyle.Quart), {
                BackgroundColor3 = Color3.fromRGB(70, 130, 180),
                TextColor3 = Color3.fromRGB(255, 255, 255)
            })
            tween:Play()
