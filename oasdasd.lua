...
function LoadDevilFruitTab()
    ...
    -----------------------------------------------------
    -- ปุ่ม ESP Fruits (สวยงาม + UNG 85% Compatible)
    -----------------------------------------------------
    local fruitEspBtn = Instance.new("TextButton", Content)
    fruitEspBtn.Size = UDim2.new(1, -20, 0, 45)
    fruitEspBtn.Text = "🍎 ESP Fruits: OFF"
    fruitEspBtn.BackgroundColor3 = Color3.fromRGB(200, 50, 50)
    fruitEspBtn.TextColor3 = Color3.new(1,1,1)
    fruitEspBtn.Font = Enum.Font.GothamBold
    fruitEspBtn.TextSize = 16
    fruitEspBtn.LayoutOrder = 1
    fruitEspBtn.AutoButtonColor = false
    
    -- สร้าง UI Corner และ Stroke สำหรับความสวยงาม
    local fruitEspCorner = Instance.new("UICorner", fruitEspBtn)
    fruitEspCorner.CornerRadius = UDim.new(0, 12)
    
    local fruitEspStroke = Instance.new("UIStroke", fruitEspBtn)
    fruitEspStroke.Color = Color3.fromRGB(255, 100, 100)
    fruitEspStroke.Thickness = 2
    fruitEspStroke.Transparency = 0.3
    
    -- Hover effect
    fruitEspBtn.MouseEnter:Connect(function()
        local tween = TweenService:Create(fruitEspBtn, TweenInfo.new(0.2), {
            BackgroundColor3 = Color3.fromRGB(220, 70, 70)
        })
        tween:Play()
    end)
    
    fruitEspBtn.MouseLeave:Connect(function()
        local color = fruitEspEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
        local tween = TweenService:Create(fruitEspBtn, TweenInfo.new(0.2), {
    ...
    fruitEspBtn.MouseButton1Click:Connect(function()
        ...
        -- เปลี่ยนสีและ stroke ตามสถานะ
        local bgColor = fruitEspEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
        local strokeColor = fruitEspEnabled and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
        
        local bgTween = TweenService:Create(fruitEspBtn, TweenInfo.new(0.3), {BackgroundColor3 = bgColor})
        local strokeTween = TweenService:Create(fruitEspStroke, TweenInfo.new(0.3), {Color = strokeColor})
...
-- ปุ่ม Auto Claim Sam (สวยงาม + UNG 85% Compatible)
local autoClaimSamBtn = Instance.new("TextButton", Content)
autoClaimSamBtn.Size = UDim2.new(1, -20, 0, 45)
autoClaimSamBtn.Text = "💰 Auto Claim Sam: " .. (States.Fruit.AutoClaimSam and "ON" or "OFF")
autoClaimSamBtn.BackgroundColor3 = States.Fruit.AutoClaimSam and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
autoClaimSamBtn.TextColor3 = Color3.new(1,1,1)
autoClaimSamBtn.Font = Enum.Font.GothamBold
autoClaimSamBtn.TextSize = 16
autoClaimSamBtn.LayoutOrder = 2
autoClaimSamBtn.AutoButtonColor = false

-- สร้าง UI Corner และ Stroke สำหรับความสวยงาม
local samCorner = Instance.new("UICorner", autoClaimSamBtn)
samCorner.CornerRadius = UDim.new(0, 12)

local samStroke = Instance.new("UIStroke", autoClaimSamBtn)
samStroke.Color = States.Fruit.AutoClaimSam and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
samStroke.Thickness = 2
samStroke.Transparency = 0.3

-- Hover effect
autoClaimSamBtn.MouseEnter:Connect(function()
    local tween = TweenService:Create(autoClaimSamBtn, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(220, 70, 70)
    })
    tween:Play()
end)

autoClaimSamBtn.MouseLeave:Connect(function()
    local color = States.Fruit.AutoClaimSam and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
    local tween = TweenService:Create(autoClaimSamBtn, TweenInfo.new(0.2), {
        BackgroundColor3 = color
    })
    tween:Play()
...
autoClaimSamBtn.MouseButton1Click:Connect(function()
    ...
    -- เปลี่ยนสีและ stroke ตามสถานะ
    local bgColor = States.Fruit.AutoClaimSam and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
    local strokeColor = States.Fruit.AutoClaimSam and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
    
    local bgTween = TweenService:Create(autoClaimSamBtn, TweenInfo.new(0.3), {BackgroundColor3 = bgColor})
    local strokeTween = TweenService:Create(samStroke, TweenInfo.new(0.3), {Color = strokeColor})
...
-- ปุ่ม Drop Compass (สวยงาม + UNG 85% Compatible)
local dropCompassBtn = Instance.new("TextButton", Content)
dropCompassBtn.Size = UDim2.new(1, -20, 0, 45)
dropCompassBtn.Text = "🗺️ Drop Compass: " .. (dropCompassEnabled and "ON" or "OFF")
dropCompassBtn.BackgroundColor3 = dropCompassEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
dropCompassBtn.TextColor3 = Color3.new(1,1,1)
dropCompassBtn.Font = Enum.Font.GothamBold
dropCompassBtn.TextSize = 16
dropCompassBtn.LayoutOrder = 3
dropCompassBtn.AutoButtonColor = false

-- สร้าง UI Corner และ Stroke สำหรับความสวยงาม
local compassCorner = Instance.new("UICorner", dropCompassBtn)
compassCorner.CornerRadius = UDim.new(0, 12)

local compassStroke = Instance.new("UIStroke", dropCompassBtn)
compassStroke.Color = dropCompassEnabled and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
compassStroke.Thickness = 2
compassStroke.Transparency = 0.3

-- Hover effect
dropCompassBtn.MouseEnter:Connect(function()
    local tween = TweenService:Create(dropCompassBtn, TweenInfo.new(0.2), {
        BackgroundColor3 = Color3.fromRGB(220, 70, 70)
    })
    tween:Play()
end)

dropCompassBtn.MouseLeave:Connect(function()
    local color = dropCompassEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
    local tween = TweenService:Create(dropCompassBtn, TweenInfo.new(0.2), {
        BackgroundColor3 = color
    })
    tween:Play()
...
dropCompassBtn.MouseButton1Click:Connect(function()
    ...
    -- เปลี่ยนสีและ stroke ตามสถานะ
    local bgColor = dropCompassEnabled and Color3.fromRGB(50, 200, 50) or Color3.fromRGB(200, 50, 50)
    local strokeColor = dropCompassEnabled and Color3.fromRGB(100, 255, 100) or Color3.fromRGB(255, 100, 100)
    
    local bgTween = TweenService:Create(dropCompassBtn, TweenInfo.new(0.3), {BackgroundColor3 = bgColor})
    local strokeTween = TweenService:Create(compassStroke, TweenInfo.new(0.3), {Color = strokeColor})
